# False: duplicate numbers (other than 0) along a row, a column, or a 3x3 subgrid
# Create a helper function that checks for duplicate in a list
# For rows, check each list in the board
# For columns, create new lists with zip(*board) and check each list
# For subgrids, iterate through 3 rows and columns each time and check duplicates
...
def valid_sudoku(grid):
    ...
    for subgrid_row in range(0, 9, 3):
        for subgrid_col in range(0, 9, 3):
            ...
            for row in range(3):
                for col in range(3):
                    subgrid.append(grid[subgrid_row + row][subgrid_col + col])
